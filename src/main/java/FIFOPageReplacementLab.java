...
public class FIFOPageReplacementLab {
     ...
     *
     * Simulates FIFO page replacement for a given reference string and number of
     * frames.
     * Returns an array: [pageFaults, pageHits]
     *
     * Algorithm:
     * 1. Create a Queue<Integer> to represent frames in memory (use LinkedList)
     * 2. Create a Set<Integer> to quickly check if a page is in memory (use
     * HashSet)
    ...
    public static int[] simulateFIFO(int[] referenceString, int numFrames) {
        ...
        Queue<Integer> Framesqueue = new LinkedList<>();
        ...
        for (int page : referenceString) {
            if (pagesInMemory.contains(page)) {
                ...
                if (Framesqueue.size() == numFrames) {
                    int victim = Framesqueue.poll();
                    pagesInMemory.remove(victim);
                }
                Framesqueue.offer(page);
